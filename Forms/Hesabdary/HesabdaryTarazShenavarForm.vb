...
Namespace Negar.Forms
    Partial Class HesabdaryTarazShenavarForm
        ...
        Private Sub HesabdaryTarazShenavarForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
            ...
            Dim allItems As String() = {"گروه (بستن همه)", "سطح ۱", "سطح ۲", "سطح ۳", "سطح ۴", "سطح ۵"}
        ...
        Private Sub LoadData()
            ...
            Using progress As New ProgressForm()
                ...
                progress.UpdateProgress(15, "محاسبه تراز شناور و دریافت اطلاعات تفصیلی‌ها...")

                Try
                    dt = service.GetAllShenavarsWithDirectTotals(fromDateStr, toDateStr, fromDoc, toDoc, docStatus, allFiscalYears)
                ...
                progress.UpdateProgress(50, "تحلیل و آماده‌سازی گره‌های تراز شناور...")
                ...
                progress.UpdateProgress(75, "برقراری روابط سلسله‌مراتبی...")
                ...
                progress.UpdateProgress(90, "محاسبه جمع‌های تجمعی...")
                ...
                progress.UpdateProgress(100, "تراز شناور آماده شد")
            End Using
        End Sub
        ...
        Private Sub DgvTrial_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvTrial.CellContentClick
            ...
            If dgvTrial.Columns(e.ColumnIndex).Name = "colLedger" Then
                ...
                If node IsNot Nothing Then
                    ...
                    RaiseEvent ShenavarSelected(node.AccountID, node.AccountCode, node.AccountName, node.HasChildren, allIds)
                End If
            End If
        End Sub
        ...
        Private Sub HesabdaryTarazShenavarForm_Shown(sender As Object, e As EventArgs) Handles MyBase.Shown
        ...
        Private Sub btnPrintTaraz_Click(sender As Object, e As EventArgs) Handles btnPrintTaraz.Click
            Try
                ...
                If dgvTrial.Columns("colCode") IsNot Nothing AndAlso dgvTrial.Columns("colCode").Visible Then
                    printCols.Add(New HesabdaryTarazPrintForm.PrintColumnInfo() With {.Key = "AccountCode", .Title = "کد شناور", .WidthRatio = 1.5F})
                End If
                printCols.Add(New HesabdaryTarazPrintForm.PrintColumnInfo() With {.Key = "AccountName", .Title = "نام تفصیلی شناور", .WidthRatio = 3.5F})
                ...
                Using printForm As New HesabdaryTarazPrintForm(companyName, "تراز آزمایشی تفصیلی شناور (" & dateTitle & ")", printCols, printRows, totals)
        ...
        Private Sub btnExportExcel_Click(sender As Object, e As EventArgs) Handles btnExportExcel.Click
            ExportGridToExcel(dgvTrial, "Floating_Trial_Balance")
        End Sub
        ...
        Private Sub UpdateHeaderTitleForSelectedRow()
            ...
            If node IsNot Nothing Then
                ...
                If Not String.IsNullOrEmpty(node.StandardAccountCode) Then
                    Try
                        Dim stdAccIdObj = Sql.ExecuteScalar("SELECT AccountID FROM SarfaslHesab WHERE CompanyID = ? AND AccountCode = ?", SessionContext.CurrentCompanyID.Value, node.StandardAccountCode)
