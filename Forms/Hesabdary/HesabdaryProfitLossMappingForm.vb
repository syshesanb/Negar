...
Namespace Sys_Hes_Anb.Forms
    Public Class HesabdaryProfitLossMappingForm
        ...
        ' Dictionary to hold mappings in memory before saving
        ' Key: CategoryKey, Value: HashSet of AccountIDs
        ...
        Private _categories As New Dictionary(Of String, String) From {
            {"SALES", "فروش / درآمدها"},
            {"SALES_RETURNS", "کسری‌ها و برگشتی‌های فروش"},
            {"INV_START", "--- موجودی کالای اول دوره"},
            {"PURCHASES", "--- خرید کالا"},
            {"PURCHASE_RETURNS", "--- کسری‌ها و برگشتی‌های خرید"},
            {"DIRECT_COSTS", "--- دستمزد و سربار مستقیم تولید"},
            {"INV_END", "--- (کسر می‌شود) موجودی پایان دوره"},
            {"OP_EXPENSES", "هزینه‌های عملیاتی / اداری و تشکیلاتی"},
            {"NON_OP", "سایر درآمدها و هزینه‌های غیرعملیاتی"},
            {"TAX", "مالیات"}
        }
        ...
        Private Sub InitializeComponent()
            Me.Text = "معرفی حساب‌های سود و زیانی"
        ...
        Private Sub LoadAccountsTree()
            ...
            Dim dt As DataTable = Sql.ExecuteTable("SELECT * FROM SarfaslHesab WHERE CompanyID = " & SessionContext.CurrentCompanyID.Value & " ORDER BY AccountCode")
        ...
        Private Sub LoadExistingMappings()
            If Not SessionContext.CurrentCompanyID.HasValue Then Return
            Dim dt As DataTable = Sql.ExecuteTable("SELECT * FROM PnLAccountMappings WHERE CompanyID = " & SessionContext.CurrentCompanyID.Value)
            For Each row As DataRow In dt.Rows
        ...
        Private Sub tvAccounts_AfterCheck(sender As Object, e As TreeViewEventArgs)
            ...
            ' Optionally auto-check children
            _isUpdatingChecks = True
