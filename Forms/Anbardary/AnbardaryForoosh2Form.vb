    ...
    Public Class AnbardaryForoosh2Form
        ...
        Private Sub AnbardaryForoosh2Form_Load(sender As Object, e As EventArgs) Handles MyBase.Load
            ...
            If Me.dgvLines IsNot Nothing Then
                Me.dgvLines.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(255, 242, 248)
            End If
            ...
            Else
                ' هدر پیش‌فرض جدید
                txtInvoiceNumber.Text = "SINV-" & DateTime.Now.ToString("yyyyMMddHHmmss")
        ...
        Private Sub LoadInvoiceForEdit(invoiceId As Integer)
            Try
                ...
                txtPartyName.Text = Convert.ToString(hdr("CustomerName"))
                If Not Convert.IsDBNull(hdr("WarehouseID")) Then
                    cmbWarehouse.SelectedValue = Convert.ToInt32(hdr("WarehouseID"))
                End If

                ' بارگذاری جزئیات
                _linesTable = _invoiceService.GetSalesInvoiceDetails(invoiceId)
                dgvLines.DataSource = _linesTable
                RefreshTotal()
        ...
        Private Sub BtnAddLine_Click(sender As Object, e As EventArgs) Handles btnAddLine.Click
            ...
            ' بررسی موجودی انبار برای فروش
            Dim productId = Convert.ToInt32(cmbProduct.SelectedValue)
            Dim warehouseId = Convert.ToInt32(cmbWarehouse.SelectedValue)
            Dim availableQtyVal = Sys_Hes_Anb.Data.Sql.ExecuteScalar("SELECT Quantity FROM Inventory WHERE ProductID = ? AND WarehouseID = ?", productId, warehouseId)
            ...
            ' اگر ویرایش می‌کنیم، مقدار قدیمی خودش را هم به موجودی اضافه کنیم تا ببینیم چقدر در کل مجاز است
            If _editInvoiceId.HasValue Then
                Dim oldQtyVal = Sys_Hes_Anb.Data.Sql.ExecuteScalar("SELECT Quantity FROM SalesInvoiceDetails WHERE InvoiceID = ? AND ProductID = ?", _editInvoiceId.Value, productId)
            ...
            If quantity > availableQty Then
                MessageBox.Show("موجودی کالا در این انبار کافی نیست. موجودی فعلی: " & availableQty.ToString("N2"), "کسری موجودی", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                txtQuantity.Focus()
                Return
            End If
            ...
            For Each row As DataRow In _linesTable.Rows
                If row.RowState <> DataRowState.Deleted AndAlso Convert.ToInt32(row("ProductID")) = productId Then
                    MessageBox.Show("این کالا قبلاً به فاکتور اضافه شده است.", "هشدار", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    Return
                End If
            Next
        ...
        Private Sub BtnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
            ...
            If String.IsNullOrWhiteSpace(txtPartyName.Text) Then
                MessageBox.Show("نام خریدار الزامی است.", "خطا", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                txtPartyName.Focus()
                Return
            End If
            ...
            Try
                ...
                If _editInvoiceId.HasValue Then
                    _invoiceService.UpdateSalesInvoice(_editInvoiceId.Value, txtInvoiceNumber.Text.Trim(), dtpInvoiceDate.Value, txtPartyName.Text.Trim(), warehouseId, createdBy, lines)
                    MessageBox.Show("فاکتور فروش با موفقیت ویرایش شد.", "موفقیت", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Else
                    Dim newId = _invoiceService.SaveSalesInvoice(txtInvoiceNumber.Text.Trim(), dtpInvoiceDate.Value, txtPartyName.Text.Trim(), warehouseId, createdBy, lines)
                    MessageBox.Show("فاکتور فروش جدید با شماره " & txtInvoiceNumber.Text.Trim() & " ثبت شد.", "موفقیت", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
