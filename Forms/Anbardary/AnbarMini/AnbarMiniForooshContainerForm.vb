...
Namespace Negar.Forms.Anbardary.AnbarMini
    Public Class AnbarMiniForooshContainerForm
        Inherits Form
        ...
        Private WithEvents forooshForm As AnbarMiniForooshForm
        ...
        Private Sub InitializeComponents()
            ...
            ' Instantiate Foroosh Form inside Edit Container
            forooshForm = New AnbarMiniForooshForm()
            forooshForm.TopLevel = False
            forooshForm.FormBorderStyle = FormBorderStyle.None
            forooshForm.Dock = DockStyle.Fill
            forooshForm.Visible = True
            pnlEditContainer.Controls.Add(forooshForm)
            ...
            ' Footer Panel for Grand Total Sales
            ...
            AddHandler Me.Load, AddressOf AnbarMiniForooshContainerForm_Load
            AddHandler Me.VisibleChanged, AddressOf AnbarMiniForooshContainerForm_VisibleChanged
        End Sub
        ...
        Private Sub SetupGridColumns()
            ...
            Dim colCustomer As New DataGridViewTextBoxColumn() With {
                .Name = "CustomerName", .DataPropertyName = "CustomerName",
                .HeaderText = "خریدار / مشتری", .Width = 180
            }

            Dim colWh As New DataGridViewTextBoxColumn() With {
                .Name = "WarehouseName", .DataPropertyName = "WarehouseName",
                .HeaderText = "انبار مبدا", .Width = 130
            }
            ...
            dgvInvoices.Columns.AddRange(New DataGridViewColumn() {
                colRowIdx, colEdit, colDelete, colId, colNo, colDate, colCustomer, colWh, colTotal, colPayType, colSanadRef, colDesc
            })
        End Sub
        ...
        Private Sub AnbarMiniForooshContainerForm_Load(sender As Object, e As EventArgs)
            ShowListView()
        End Sub

        Private Sub AnbarMiniForooshContainerForm_VisibleChanged(sender As Object, e As EventArgs)
        ...
        Private Sub LoadInvoicesList()
            Try
                dtInvoices = invoiceService.GetSalesInvoices()
            ...
            Catch ex As Exception
                Console.WriteLine("Error loading sales invoices list: " & ex.Message)
            End Try
        End Sub
        ...
        Private Sub ApplyFilter()
            ...
            If Not String.IsNullOrEmpty(f) Then
                filters.Add($"(InvoiceNumber LIKE '%{f}%' OR CustomerName LIKE '%{f}%' OR WarehouseName LIKE '%{f}%')")
            End If
        ...
        Private Sub RecalculateTotalSales()
        ...
        Private Sub ShowEditView()
            forooshForm.ResetForm()
            pnlListContainer.Visible = False
            pnlEditContainer.Visible = True
            pnlEditContainer.BringToFront()
        End Sub

        Private Sub forooshForm_InvoiceSaved() Handles forooshForm.InvoiceSaved
            ShowListView()
        End Sub

        Private Sub forooshForm_InvoiceCancelled() Handles forooshForm.InvoiceCancelled
            ShowListView()
        End Sub
        ...
        Private Sub DgvInvoices_CellContentClick(sender As Object, e As DataGridViewCellEventArgs)
            If e.RowIndex >= 0 Then
                ...
                ElseIf colName = "colDelete" Then
                    ...
                    If MessageBox.Show($"آیا از حذف فاکتور فروش {invNo} اطمینان دارید؟", "تایید حذف", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                        Try
                            invoiceService.DeleteSalesInvoice(invId)
                            MessageBox.Show("فاکتور فروش با موفقیت حذف گردید.", "اطلاع", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            LoadInvoicesList()
                        Catch ex As Exception
