...
Namespace Negar.Business
    Public Class ShenavarService
        ...
        Public Function GetByParent(parentId As Integer?) As DataTable
            ...
            If parentId.HasValue Then
                Return Sql.ExecuteTable(
                    "SELECT ShenavarID AS AccountID, AccountCode, AccountName, " &
                    "ParentShenavarID AS ParentAccountID, IsActive " &
                    "FROM SarfaslShenavar WHERE CompanyID = ? AND ParentShenavarID = ? ORDER BY AccountCode",
                    companyId, parentId.Value)
            Else
                Return Sql.ExecuteTable(
                    "SELECT ShenavarID AS AccountID, AccountCode, AccountName, " &
                    "ParentShenavarID AS ParentAccountID, IsActive " &
                    "FROM SarfaslShenavar WHERE CompanyID = ? AND ParentShenavarID IS NULL ORDER BY AccountCode",
                    companyId)
            End If
        End Function
        ...
        Public Function Save(itemId As Integer?, accountCode As String, accountName As String,
            ...
            If parentId.HasValue Then
                dupCount = Convert.ToInt32(If(Sql.ExecuteScalar(
                    "SELECT COUNT(*) FROM SarfaslShenavar WHERE CompanyID = ? AND AccountCode = ? AND ParentShenavarID = ? AND ShenavarID <> ?",
                    companyId, accountCode, parentId.Value, excludeId), 0))
            Else
                dupCount = Convert.ToInt32(If(Sql.ExecuteScalar(
                    "SELECT COUNT(*) FROM SarfaslShenavar WHERE CompanyID = ? AND AccountCode = ? AND ParentShenavarID IS NULL AND ShenavarID <> ?",
            ...
            Return Sql.ExecuteIdentity(
                "INSERT INTO SarfaslShenavar (CompanyID, AccountCode, AccountName, ParentShenavarID, IsActive) VALUES (?, ?, ?, ?, ?)",
        ...
        Public Sub Delete(itemId As Integer)
            ...
            Sql.ExecuteNonQuery("DELETE FROM SarfaslShenavar WHERE ShenavarID = ? AND CompanyID = ?",
        ...
        Public Function HasChildren(itemId As Integer) As Boolean
            ...
            Dim count = Convert.ToInt32(If(Sql.ExecuteScalar(
                "SELECT COUNT(*) FROM SarfaslShenavar WHERE CompanyID = ? AND ParentShenavarID = ?",
        ...
        Public Function GetParentId(itemId As Integer) As Integer?
            Dim result = Sql.ExecuteScalar(
                "SELECT ParentShenavarID FROM SarfaslShenavar WHERE ShenavarID = ?", itemId)
        ...
        Public Function GetName(itemId As Integer) As String
            Dim result = Sql.ExecuteScalar(
                "SELECT AccountName FROM SarfaslShenavar WHERE ShenavarID = ?", itemId)
        ...
        Public Function GetItemInfo(itemId As Integer) As Tuple(Of String, String)
            Dim dt = Sql.ExecuteTable(
                "SELECT AccountCode, AccountName FROM SarfaslShenavar WHERE ShenavarID = ?", itemId)
        ...
        Public Function GetNextSuggestedCode(parentId As Integer?) As String
            ...
            If parentId.HasValue Then
                maxCodeObj = Sql.ExecuteScalar(
                    "SELECT MAX(AccountCode) FROM SarfaslShenavar WHERE CompanyID = ? AND ParentShenavarID = ?",
                    companyId, parentId.Value)
            Else
                maxCodeObj = Sql.ExecuteScalar(
                    "SELECT MAX(AccountCode) FROM SarfaslShenavar WHERE CompanyID = ? AND ParentShenavarID IS NULL",
                    companyId)
            End If
        ...
        Public Function SearchAll(codeFilter As String, nameFilter As String) As DataTable
            ...
            Dim query = "SELECT ShenavarID AS AccountID, AccountCode, AccountName, " &
                        "ParentShenavarID AS ParentAccountID, IsActive " &
                        "FROM SarfaslShenavar WHERE CompanyID = ?"
        ...
        Public Function GetShenavarsWithChildren() As HashSet(Of Integer)
            ...
            Dim dt = Sql.ExecuteTable(
                "SELECT DISTINCT ParentShenavarID FROM SarfaslShenavar WHERE CompanyID = ? AND ParentShenavarID IS NOT NULL",
